...
const getPaymentForClasses = async (req, res) => {
 ...
 async function checkPromoCode(promo_code) {
    ...
    const customerSeeClasses = async (req, res) => {
        ...
        pool.query(
            "SELECT fitness_class.*, gym.*, studio.*, class_type.*, instructor.First_Name, instructor.Last_Name, instructor.Gender, instructor.Languages FROM power_pals.fitness_class as fitness_class, power_pals.instructor as instructor, power_pals.gym as gym, power_pals.class_type as class_type, power_pals.studio as studio WHERE fitness_class.Gym_ID = gym.Gym_ID AND fitness_class.Instructor_ID = instructor.Instructor_ID AND fitness_class.Class_Category = class_type.Class_Category AND fitness_class.Studio_Room_No = studio.Studio_Room_No;",
            [customerId],
            (error, results, fields) => {
                if (error) {
                    console.log(error);
